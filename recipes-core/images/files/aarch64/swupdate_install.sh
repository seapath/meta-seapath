...
do_preinst()
{
    ...
    if [[ "${part_num}" == "3" ]] ; then
        bootloader_label="boot0"
        rootfs_label="rootfs0"
    else
        bootloader_label="boot1"
...
do_postinst()
{
    ...
    if ! fw_setenv bootcount 0 ; then
        die "Could not set bootcount"
    fi

    if ! fw_setenv upgrade_available 1 ; then
        die "Could not set upgrade_available"
    fi
